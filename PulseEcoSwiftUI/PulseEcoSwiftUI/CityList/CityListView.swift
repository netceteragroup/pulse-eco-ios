...
struct CityListView: View {
    ...
    var body: some View {
        VStack {
            VStack(spacing: 5) {
                Text(Trema.text(for: "search_city_or_choose_suggested", lang: UserDefaults.standard.string(forKey: "AppleLanguage") ?? "en"))
                    .foregroundColor(Color.white).multilineTextAlignment(.center)
                SearchBar(text: self.$viewModel.searchText,
                          placeholder: Trema.text(for: "search_city_or_country", lang: UserDefaults.standard.string(forKey: "AppleLanguage") ?? "en"))
